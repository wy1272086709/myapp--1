					...
					<view id="cases-content">
						<view class="cview padding-css" v-for="(item,index, index2) in casesNavList" :key="index">
							...
							<view class="content-view">
								<view v-for="(info,m) in item" class="content-root-view" :style="index2%2==0?'flex-direction:row':'flex-direction:row-reverse'"  @tap="getArticleView(info.id, info.cid, item[0].classname)" :key="info.id">
	...
	export default {
		...
		methods: {
			...
			async getCategoryArr() {
				...
				this.casesNavList = Object.freeze(respData);
				let s2 = new Date();
