<template>
	<view id="root-view">
		...
		<u-tabs v-if="control" gutter="33" bg-color="#FFFFFF" :name="'title'" :bold="bold" :active-color="activeColor" :list="tabs"
		@change="change" :bar-width="110" :current="caseTabIndex" :is-scroll="isScroll"></u-tabs>
		
		<view>			
			<swiper :current="caseTabIndex" :scroll-with-animation="true" 	@change="switchCasesTab" :style="'min-height:'+scrollHeight+'px;width:100%;'">
				<swiper-item v-for="(tab,index) in tabArr" :key="tab.firstNav.navId" :style="'min-height:'+scrollHeight+'px;'">
					<scroll-view id="content-view"   enable-back-to-top="true" :style="'height:'+scrollHeight+'px;'" :scroll-y="true" @scrolltoupper="upper" @scrolltolower="lower">
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.scrollHeight  = winHeight - uni.upx2px(81+bottom);
		...
		methods: {
			...
			getHuashuTabbar(cId) {
				...
				http.request(url, {
					...
					const n = resp.length;
					for(let m = 0;m<n;m++) {
						const title = resp[m].firstNav.title;
